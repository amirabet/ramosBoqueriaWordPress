...
function wordimpress_preview_woo_emails() {
	...
	if ( is_admin() ) {
		...
		$plugin_path  = plugin_dir_path( __FILE__ ) . '/ramos_boqueria/woocommerce/';

		//wc_get_template( 'emails/email-header.php', array( 'order' => $order, 'email_heading' => $email_heading ) );
		wc_get_template( $plugin_path . 'emails/email-header.php', array( 'order' => $order, 'email_heading' => $email_heading ) );
		...
		wc_get_template( $plugin_path . 'emails/email-footer.php', array( 'order' => $order ) );
	}
}
